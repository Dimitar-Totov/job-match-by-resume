...
Vite + React 19 + TypeScript SPA for an AI resume/job-match analyzer. There's no backend of our own — Supabase is used directly as the auth provider (see below). Routing is a hybrid: `react-router-dom` owns the five pre-dashboard/entry URLs, everything past that is hand-rolled state (see below). No styling library or fetch library is used — don't introduce Tailwind or a fetch library without explicit instruction.

### Navigation is a router for auth entry points, state for everything past the dashboard

`react-router-dom` (`BrowserRouter` in `src/main.tsx`) owns exactly five routes, defined in `src/App.tsx` via `PATHS` (`src/routes/paths.ts`): `/` (welcome), `/register`, `/login`, `/onboarding`, `/dashboard`. Each has a dedicated route-element component in `src/pages/` (`WelcomeRoute`, `RegisterRoute`, `LoginRoute`, `OnboardingRoute`, `DashboardRoute`) that owns the sign-up/sign-in submit handlers and their loading/error state — mirrors what the now-deleted `RootView.tsx` used to do in one file, just split per route. Email (welcome → register) and pending registration data (register → onboarding) travel via React Router's `navigate(path, { state })`, typed as `RegisterLocationState`/`OnboardingLocationState` with runtime type guards (no `any`) since `location.state` is `unknown`.

Every one of those five routes is guarded, based on `useAuth().status` (see below):
- `RequireGuest` (`src/routes/RequireGuest.tsx`) wraps `/`, `/register`, `/login`, `/onboarding` — redirects to `/dashboard` if already authenticated.
- `RequireAuth` (`src/routes/RequireAuth.tsx`) wraps `/dashboard` — redirects to `/login` if a guest.
...
Everything **inside** `/dashboard` still uses the pre-router system, unchanged: `DashboardRoute` mounts `NavProvider` once and renders `AppShell` inside it, so the URL bar stays on `/dashboard` no matter what the user clicks. `NavContext`/`NavProvider` (`src/context/`) hold the current `Screen` (`src/types/index.ts`) and a `navigate(screen, options?)` setter, plus sidebar-collapsed state. Consume via the `useNav()` hook (`src/hooks/useNav.ts`) — never import `NavContext` directly in components (that split exists to satisfy Oxlint's `react/only-export-components`). `NavProvider` mirrors every screen change into browser history via `pushState`/`replaceState` and listens for `popstate`; in-app Back controls should call `window.history.back()` rather than `navigate()` directly to keep that one popstate handler as the single source of truth. `navigate(screen, { replace: true })` swaps the current history entry instead of pushing.

`src/features/app-shell/navConfig.ts` is the single source of truth for sidebar nav items (`NAV_SECTIONS`, `BOTTOM_NAV`) and the per-screen page title (`SCREEN_TITLES`). Adding a new **in-shell** screen means: add it to the `Screen` union, add a section/page component under `src/features/<name>/`, wire it into `AppShell`'s switch, and add an entry to `navConfig.ts` if it should be reachable from the sidebar. Adding a new **top-level, pre-dashboard** route (rare) means: add a path to `PATHS`, a route-element component in `src/pages/`, a `<Route>` entry in `App.tsx` wrapped in the appropriate guard.
